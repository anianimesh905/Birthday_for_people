...
└── public/
    ...
    │
    └── js/
        ├── core/         <- State namespaces, event coordinators, bootstrappers, helpers
...
- **`bootstrap.js`**: Exposes `onDOMReady` wrapper utilizing `{ once: true }` DOMContentLoaded listener to safely handle load-order race conditions.
- **`constants.js`**: Holds read-only settings (colors, default sizes, spell names, emoji listings, and character coordinates).
- **`state.js`**: Centralized domain-specific state namespaces (`state.audio`, `state.story`, `state.pointer`, `state.spells`, `state.castle`, `state.environment`, `state.ui`, `state.system`). Contains zero executable callbacks.
- **`events.js`**: normalized single source coordinator for browser window events (resize, visibility, focus, keyboard, network, motion query preferences). Runs a continuous 60fps LERP loop to interpolate smooth pointer movements (`x`/`y` towards `targetX`/`targetY`).
...
- **`buttons.js`**: Manages music play/pause toggling, exposing direct `startMusic` calls.
- **`parallax.js`**: Leverages smoothed `state.pointer` coordinates to shift CSS variables `--mx`/`--my`.
- **`spells.js`**: Processes typing spells, triggering overlays like Glacius scratch canvas, Fumos mist, and Accio card rotations.
- **`particles.js`**: Manages the `<canvas id="sparkle-canvas">` trail engine, exposing `spawnSparkCluster` for external callers.
...
sequenceDiagram
    ...
    Browser->>main.js: Load ES6 module
    main.js->>bootstrap.js: onDOMReady(init)
    activate bootstrap.js
    bootstrap.js->>main.js: Execute init callback
    deactivate bootstrap.js
    ...
    preloader.js->>events.js: initCentralEvents()
...
1. **Bootstrap Phase**: Browser loads `index.html` and parses `content.js` to create the global config object. It registers `public/js/main.js` as an ES6 module, which schedules initialization using `onDOMReady` to ensure the DOM tree is safe before querying elements.
...
3. **Core Activation Phase**: On download completion (or 15s timeout), the preloader fades out. `initCentralEvents()` binds all window event listeners once, `initVideoBackground()` creates the background video buffer, `initAmbientAtmosphere()` initializes the background canvas loop, and `initEnvelope()` configures card listeners.
