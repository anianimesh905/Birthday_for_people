...
export function initMagicParticles(opts = {}) {
  ...
  if (state.ui.currentHouse && PALETTES[state.ui.currentHouse]) {
    palette = { ...PALETTES[state.ui.currentHouse] };
  }
  ...
  window.addEventListener('mousemove', () => {
    if (state.spells.isLumosActive || (state.spells.activeMode && state.spells.activeMode !== 'none')) {
      startParticlesLoop();
    }
  }, { passive: true });
  ...
  window.addEventListener('envelopeTapped', (e) => {
    const cx = e.detail && e.detail.x;
    const cy = e.detail && e.detail.y;
    let emitted = 0;
    const target = isMobile ? 12 : 26;
    for (let i = 0; i < 40; i++) {
      ...
      if (p) {
        p.active = true;
        p.type = 'A';
        p.x = cx !== undefined ? cx : rand(0, canvas.width);
        p.y = cy !== undefined ? cy : rand(0, canvas.height);
